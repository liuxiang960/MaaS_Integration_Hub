...
export function removeListItem(list, ids, dt) {
  const mulist = []
  for (let i = 0; i < list.length; i++) {
    ...
    if (item.id != ids) {
      ...
      if (dt && dt != undefined && dt != null && dt != "undefined") {
        mulist.push(dt)
      }
    }
  }
...
export function getTimeSub(nS) {
  ...
  const d = date.getDate() < 10 ? "0" + date.getDate() : date.getDate() // 获取日(1-31)

  return y + "-" + m + "-" + d
}

export function getDays(n) {
  const time = 1610176741
